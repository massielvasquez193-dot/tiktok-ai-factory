...
## [1.0.0] — 2026-06-07 — Production Release 🚀

### Overview
Enterprise SaaS platform with JWT auth, Stripe payments, multi-tenant architecture, credit system, admin dashboard with real-time charts, Docker deployment with SSL + auto-backup.

### Release Notes → [PROJECT_RELEASE_v1.0.md](PROJECT_RELEASE_v1.0.md)

### Added — SaaS Platform
- `apps/server/src/auth/` — JWT 认证模块 (注册/登录/Token/Refresh/密码加密/邮箱验证/角色中间件)
- `apps/server/src/payments/` — Stripe 支付模块 (订阅/一次性支付/Webhook/支付记录/客户门户)
- `apps/server/src/credits/` — 额度系统模块 (余额/冻结/扣减/账本/管理员调整)
- `apps/server/src/tenant/` — 多租户模块 (CRUD/成员管理/邀请/计划限制)
- Prisma SaaS 模型: User, Tenant, TenantMember, Subscription, Payment, CreditWallet, CreditLedger, CreditPack, EmailVerificationToken
- `apps/web/src/context/AuthContext.tsx` — 前端认证状态管理
- `apps/web/src/app/login/` — 登录页面
- `apps/web/src/app/register/` — 注册页面
- `apps/web/src/app/settings/` — 账户设置 (额度/账单/个人信息)
- `apps/web/src/app/admin/` — 管理后台 (仪表盘/用户管理/租户管理/额度管理/支付管理)
- `apps/web/src/components/Sidebar.tsx` — 基于角色的侧边栏导航
- `apps/server/src/__tests__/` — 集成测试 (Auth + Credits)
- `deploy-saas.ps1` — 一键 SaaS 部署脚本
- `docs/SaaS_DEPLOYMENT.md` — SaaS 部署文档 + API 参考

### Changed
- `apps/server/src/index.ts` — 注册 auth/payments/credits/tenant 路由 + Stripe raw body 处理
- `apps/server/prisma/schema.prisma` — Product 表新增 tenantId 外键
- `apps/web/src/app/layout.tsx` — 集成 AuthProvider
- `.env` — 新增 JWT/Stripe/Email 环境变量
